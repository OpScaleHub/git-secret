...
5. **A ciphertext decrypts only where it was sealed.** For a `GitSecret` that is
   the exact object/namespace/key (`internal/sealer.aad`; `TestUnsealWrongObjectFails`).
   For a whole-file blob it is the repo-relative path, plus — when the repo has a
   `repo_id` — the repository itself, so a blob is not portable between repos that
   share a key (`internal/cli.fileBindAAD`; envelope v2, `crypto/envelope.go`;
   `TestSealFile_RepoBoundBlobNotPortable`). The AAD binds *identity*, not
   *version* — see T10-CLI for what this does not cover.
