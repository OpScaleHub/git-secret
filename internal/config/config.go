...
type Config struct {
	...
	// RepoID is a random per-repository identifier, generated by `init`
	// and committed. Whole-file encryption folds it into the AEAD's
	// additional-authenticated-data (a v2 envelope), so a ciphertext blob
	// sealed here fails authentication if dropped into another repository
	// at the same path -- even one that happens to share the key. Empty on
	// repos initialised before this existed: those keep sealing v1
	// (path-only AAD) and stay fully readable. Never change it by hand --
...
func NewRepoID() (string, error) {
	...
	if _, err := rand.Read(b); err != nil {
		return "", fmt.Errorf("config: read random for repo_id: %w", err)
	}
	return hex.EncodeToString(b), nil
}

// validRepoID matches the output of NewRepoID plus a little slack for
// hand-written ids.
var validRepoID = regexp.MustCompile(`^[A-Za-z0-9_.-]{8,64}$`)
...
func mergeInto(base, overlay *Config) {
	...
	if overlay.RepoID != "" {
		// repo-local only; a global override never contributes a repo id.
		base.RepoID = overlay.RepoID
	}
